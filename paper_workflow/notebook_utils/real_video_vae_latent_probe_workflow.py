...
def materialize_probe_family_results_to_drive(
    *,
    ...
    该函数属于 notebook 交付层的通用写法。主要考虑在于避免正式运行失败时提前
    在 Google Drive 的结果目录下创建空 family 文件夹。调用方应先在 session-local
    family root 中完成 checker、mechanism audit、zip/tar.zst 打包和最终摘要写盘,
    然后再调用本函数执行一次性 Drive 落盘与 registry 登记。

    Args:
        local_family_root: session-local family 结果目录。
        drive_family_root: Google Drive 中最终 family 结果目录。
        package_payload: package_probe_family_results 返回的本地打包摘要。
        drive_root: Google Drive 根目录, 为 None 时不登记 registry。
        family_id: family 标识, 用于 registry。
        workflow_key: workflow 标识, 用于 registry。
        step_key: step 标识, 用于 registry。
        run_mode: 运行模式。
        formal_validation_summary: formal checker 摘要。
        mechanism_summary: mechanism audit 摘要。

    Returns:
        Drive 落盘摘要, 其中路径均指向 Drive 结果目录。
    """
