...
def test_trajectory_aware_sampling_probe_notebook_delegates_to_repository_cli() -> None:
    ...
    assert "sampling_workflow.run_real_backend_runtime_validation_gate(" in notebook_text
    ...
    assert "sampling_workflow.run_external_controlled_real_generation_execution_handoff(" in notebook_text
    assert "sampling_workflow.run_external_controlled_single_real_generation_execution_run(" in notebook_text
    ...
    assert "build_trajectory_aware_sampling_real_backend_runtime_validation_gate" in workflow_text
    ...
    assert "build_trajectory_aware_sampling_external_controlled_real_generation_execution_handoff" in workflow_text
    assert "build_trajectory_aware_sampling_external_controlled_single_real_generation_execution_run" in workflow_text
    ...
    assert "TSTW_RUNTIME_VALIDATION_GATE_CONFIG_PATH" in notebook_text
    ...
    assert "TSTW_EXTERNAL_CONTROLLED_REAL_GENERATION_EXECUTION_HANDOFF_CONFIG_PATH" in notebook_text
    assert "TSTW_EXTERNAL_CONTROLLED_SINGLE_REAL_GENERATION_EXECUTION_RUN_CONFIG_PATH" in notebook_text
    ...
    assert "trajectory_aware_sampling_real_backend_runtime_validation_gate.json" in notebook_text
    assert "trajectory_aware_sampling_real_backend_runtime_validation_gate.json" in workflow_text
    assert "trajectory_aware_sampling_explicit_real_generation_transition_decision.json" in notebook_text
    assert "trajectory_aware_sampling_explicit_real_generation_transition_decision.json" in workflow_text
    assert "trajectory_aware_sampling_controlled_single_real_generation_request_scaffold.json" in notebook_text
    ...
    assert "trajectory_aware_sampling_external_controlled_real_generation_execution_handoff.json" in notebook_text
    assert "trajectory_aware_sampling_external_controlled_real_generation_execution_handoff.json" in workflow_text
    assert "trajectory_aware_sampling_external_controlled_single_real_generation_execution_run.json" in notebook_text
    assert "trajectory_aware_sampling_external_controlled_single_real_generation_execution_run.json" in workflow_text
    ...
    assert "TrajectoryAwareSamplingRealBackendRuntimeValidationGateDecision" in notebook_text
    assert "NextAllowedConstructionAfterRealBackendRuntimeValidationGate" in notebook_text
    assert "TrajectoryAwareSamplingExplicitRealGenerationTransitionDecision" in notebook_text
    assert "NextAllowedConstructionAfterExplicitRealGenerationTransitionDecision" in notebook_text
    assert "TrajectoryAwareSamplingControlledSingleRealGenerationRequestScaffoldDecision" in notebook_text
    ...
    assert "TrajectoryAwareSamplingExternalControlledRealGenerationExecutionHandoffDecision" in notebook_text
    assert "NextRequiredExternalExecutionAfterExternalControlledRealGenerationExecutionHandoff" in notebook_text
    assert "TrajectoryAwareSamplingExternalControlledSingleRealGenerationExecutionRunDecision" in notebook_text
    assert "NextRequiredExternalActionAfterExternalControlledSingleRealGenerationExecutionRun" in notebook_text
    ...
    assert "sampling_workflow.run_real_backend_runtime_validation_gate(" in code_text
    ...
    assert "sampling_workflow.run_external_controlled_real_generation_execution_handoff(" in code_text
    assert "sampling_workflow.run_external_controlled_single_real_generation_execution_run(" in code_text
    ...
    assert "real_backend_runtime_validation_gate.get(" in code_text
    ...
    assert "external_controlled_real_generation_execution_handoff.get(" in code_text
    assert "external_controlled_single_real_generation_execution_run.get(" in code_text
    ...
    assert "real_backend_runtime_validation_gate.get(" not in markdown_text
    ...
    assert "external_controlled_real_generation_execution_handoff.get(" not in markdown_text
    assert "external_controlled_single_real_generation_execution_run.get(" not in markdown_text
