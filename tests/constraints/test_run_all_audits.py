...
def test_run_all_audits_includes_required_modules() -> None:
    ...
    assert "tools.harness.audits.audit_notebook_naming_contract" in AUDIT_MODULE_NAMES
    assert "tools.harness.audits.audit_placeholder_random_fields" in AUDIT_MODULE_NAMES
    assert "tools.harness.audits.audit_test_case_constraints" in AUDIT_MODULE_NAMES
