...
def test_real_video_vae_latent_event_records_include_quality_and_temporal_payloads(tmp_path: Path) -> None:
    ...
    for event_score_record in event_score_records:
        ...
        assert event_score_record["mechanism_trace"]["decoded_video_relpath"].endswith(".mp4")
        assert event_score_record["mechanism_trace"]["attacked_video_relpath"].endswith(".mp4")
        assert event_score_record["mechanism_trace"]["decoded_video_container"] == "mp4"
        assert event_score_record["mechanism_trace"]["attacked_video_container"] == "mp4"
        assert event_score_record["mechanism_trace"]["quality_metrics_runtime"] == "real_video_frame_metrics"
        assert event_score_record["mechanism_trace"]["temporal_metrics_runtime"] == "real_video_frame_metrics"
...
def test_real_video_vae_latent_runtime_manifest_tracks_runtime_config_and_notebook_entrypoint(
    ...
    runtime_config_payload = {
        ...
        "drive_results_dir": "/content/drive/MyDrive/tstw_real_video_probe/results",
