"""运行 external_rivagan 的 Colab 真实可运行性 smoke。"""
...
from experiments.baseline_comparison_gate.rivagan_adapter import (
    ...
    SCORE_MAPPING_RULE,
    ExternalRivaGANAdapter,
    create_rivagan_probe_video,
)
...
RUN_ID_PREFIX = "external_rivagan_real_smoke"
...
def build_run_id(short_commit: str, timestamp_utc: str | None = None) -> str:
    """生成 external_rivagan real smoke 的 run_id。"""
...
def build_record(
    *,
    ...
    """把 RivaGAN 检测结果转换为统一 baseline record。"""
    ...
    if violations:
        raise ValueError(f"invalid external_rivagan smoke record: {violations}")
    return record


def run_rivagan_real_smoke(
    *,
    run_root: str | Path,
    config_dir: str | Path,
    ...
    """运行 RivaGAN 的 clean 与 H.264 单视频真实 smoke。"""
    ...
    if input_video_path is None:
        input_path = inputs_dir / "rivagan_probe_input.avi"
        create_rivagan_probe_video(input_path)
    else:
        input_path = Path(input_video_path)
        if not input_path.exists():
            raise FileNotFoundError(f"input video not found: {input_path}")

    adapter = ExternalRivaGANAdapter(
    ...
    watermarked_path = artifacts_dir / "external_rivagan_watermarked_clean.avi"
    ...
    records = [
        build_record(
            ...
            sample_id="external_rivagan_probe_clean",
    ...
    attacked_path = artifacts_dir / "external_rivagan_watermarked_h264_crf_28.mp4"
    attack = H264CompressionAttack({"crf": 28, "preset": "medium"})
    attack_metadata = attack.apply_video(watermarked_path, attacked_path, fps=20)
    ...
    records.append(
        build_record(
            ...
            source_digest=source_digest,
            payload_bits=payload_bits,
            sample_id="external_rivagan_probe_h264_crf_28",
    ...
    records_path = records_dir / "external_rivagan_real_smoke_records.jsonl"
    write_jsonl(records_path, records)
    report_path = reports_dir / "external_rivagan_real_smoke_report.md"
    report_path.write_text(build_report(records, prepare_result), encoding="utf-8")
    manifest = {
        "workflow_key": WORKFLOW_KEY,
        "run_id": run_id,
        "run_kind": "external_rivagan_real_smoke",
        ...
        "model_digest": model_digest,
        "model_weight_url": DEFAULT_RIVAGAN_WEIGHT_URL,
        "adapter_version": ADAPTER_VERSION,
...
def build_report(records: list[dict[str, Any]], prepare_result: dict[str, Any]) -> str:
    """生成 RivaGAN 真实 smoke 报告。"""
    lines = [
        "# external_rivagan real smoke report",
        "",
        "该报告只证明 RivaGAN 在当前 Colab 环境中完成了单视频真实 encode / decode smoke, 不能替代正式 fixed-FPR baseline comparison。",
        "",
    ...
    lines.extend(
        [
            ...
            "## 后续要求",
            "",
            "进入正式比较前, 仍需在统一 split、统一攻击矩阵和 calibration-only fixed-FPR 协议下生成全量 records、thresholds、tables 和 claim audit。",
        ]
    )
    return "\n".join(lines) + "\n"
...
def write_failure_summary(run_root: str | Path, exc: Exception) -> dict[str, Any]:
    ...
    failure_path = logs_dir / "external_rivagan_real_smoke_failure.json"
...
def main(argv: list[str] | None = None) -> None:
    ...
    try:
        with BaselineGpuProfileSession(
            ...
            summary = run_rivagan_real_smoke(
                run_root=args.run_root,
        ...
        if args.result_root is not None:
            destination = materialize_completed_smoke_run(
                run_root=args.run_root,
                result_root=args.result_root / "baseline_comparison_gate" / "external_rivagan" / "real_smoke",
                workflow_key="",
                run_id=summary["run_id"],
                overwrite=args.overwrite_result,
                required_relative_paths=[
                    "manifest.json",
                    "records/external_rivagan_real_smoke_records.jsonl",
                    "reports/external_rivagan_real_smoke_report.md",
