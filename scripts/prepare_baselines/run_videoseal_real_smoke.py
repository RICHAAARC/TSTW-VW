"""运行 external_videoseal 的 Colab 真实可运行性 smoke。"""
...
RUN_ID_PREFIX = "external_videoseal_real_smoke"
...
def build_run_id(short_commit: str, timestamp_utc: str | None = None) -> str:
    """生成 external_videoseal real smoke 的 run_id。"""
...
def create_synthetic_probe_video(output_path: Path, *, duration_seconds: int, fps: int, size: int) -> None:
    """创建一个只用于 baseline 可运行性 smoke 的确定性输入视频。"""
...
def build_record(
    *,
    ...
    """把 VideoSeal 检测结果转换为统一 baseline record。"""
    ...
    record = {
        ...
        "quality_metrics": {},
        "temporal_metrics": {
            "frame_count": detection_result.baseline_raw_detector_output.get("frame_count"),
    ...
    if violations:
        raise ValueError(f"invalid external_videoseal smoke record: {violations}")
    return record


def run_videoseal_real_smoke(
    *,
    run_root: str | Path,
    config_dir: str | Path,
    ...
    """运行 VideoSeal 的 clean 与 H.264 单视频真实 smoke。"""
    ...
    if input_video_path is None:
        input_path = inputs_dir / "videoseal_probe_input.mp4"
        create_synthetic_probe_video(input_path, duration_seconds=2, fps=8, size=256)
    else:
        input_path = Path(input_video_path)
        if not input_path.exists():
            raise FileNotFoundError(f"input video not found: {input_path}")

    adapter = ExternalVideoSealAdapter(
    ...
    watermarked_path = artifacts_dir / "external_videoseal_watermarked_clean.mp4"
    embed_result = adapter.embed(
    ...
    records = [
        build_record(
            ...
            source_digest=source_digest,
            payload_bits=payload_bits,
            sample_id="external_videoseal_probe_clean",
    ...
    attacked_path = artifacts_dir / "external_videoseal_watermarked_h264_crf_28.mp4"
    ...
    records.append(
        build_record(
            ...
            source_digest=source_digest,
            payload_bits=payload_bits,
            sample_id="external_videoseal_probe_h264_crf_28",
    ...
    records_path = records_dir / "external_videoseal_real_smoke_records.jsonl"
    write_jsonl(records_path, records)
    report_path = reports_dir / "external_videoseal_real_smoke_report.md"
    report_path.write_text(build_report(records, prepare_result), encoding="utf-8")
    manifest = {
        "workflow_key": WORKFLOW_KEY,
        "run_id": run_id,
        "run_kind": "external_videoseal_real_smoke",
...
def build_report(records: list[dict[str, Any]], prepare_result: dict[str, Any]) -> str:
    """生成 VideoSeal 真实 smoke 报告。"""
    lines = [
        "# external_videoseal real smoke report",
        "",
        "该报告只证明 VideoSeal 在当前 Colab 环境中完成了单视频真实 embed / detect smoke, 不能替代正式 fixed-FPR baseline comparison。",
        "",
