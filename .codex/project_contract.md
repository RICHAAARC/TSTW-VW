...
- Passing this gate requires `paper_method_comparison_table.csv`, `paper_attack_breakdown_table.csv`, `paper_sync_gain_table.csv`, `paper_external_baseline_table.csv`, `paper_roc_auc_table.csv`, `paper_quality_table.csv`, `paper_runtime_efficiency_table.csv`, `paper_visual_example_figure_data.csv`, optional `paper_temporal_quality_table.csv` when `temporal_quality_metric_probe` is available, optional `paper_attack_strength_table.csv` when `attack_strength_curve_probe` is available, optional `paper_additional_dataset_table.csv` when `additional_dataset_validation_probe` is available, exported PNG/PDF paper figures including the visual example grid when shard-run videos are available, `paper_claim_audit.csv`, `paper_submission_gap_audit.csv`, `paper_figure_manifest.json`, and `paper_artifact_gate_manifest.json`.

## Submission-Readiness Supplemental Figure Probes

- `temporal_quality_metric_probe`, `attack_strength_curve_probe`, and `additional_dataset_validation_probe` are allowed as submission-readiness supplemental probes before `submission_readiness_gate`.
- These probes must not overwrite frozen `real_video_vae_latent_probe` or `baseline_comparison_gate` artifacts.
...
- `t-LPIPS` may be marked supported only when a real LPIPS model path, model identity, and successful records are present. Missing LPIPS computation must remain an explicit failure or unsupported reason.
- `t-SSIM` may be computed from adjacent decoded real-video frames and may support temporal quality claims, but it must not support detection robustness claims.
