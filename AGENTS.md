...
8. Ambiguous stage naming patterns such as `*_v[0-9]+`, `*_p[0-9]+`, `stage[0-9]+`, `stage_[0-9]+`, `stage-[0-9]+`, `test_stage[0-9]+_*`, or `run_stage[0-9]+_*` are forbidden, except for governed `paper_workflow/Stage<index>_<Purpose>.ipynb` notebook entrypoints and `paper_workflow/notebook_utils/stage<index>_<purpose>.py` helper modules.
...
13. A single governed `paper_workflow/Stage2_Real_Video_VAE_Latent_Probe.ipynb` entrypoint may exist for transition preparation; it must delegate formal outputs to repository modules and must not direct-write records, thresholds, tables, figures, or reports.
