...
| --- | --- | --- | --- |
| constraints | 32 | 142 | default governance checks |
| functional | 37 | 154 | default quick behavior checks |
| integration | 21 | 38 | excluded runner/smoke/formal checks |
...
| `tests/constraints/test_mechanism_decision_contract.py` | 1 | constraints | `constraint`, `unit` | 1 | no | no | yes | 验证 Stage2MechanismDecision 的门禁语义与字段完整性。 |
...
| `tests/constraints/test_real_video_vae_latent_colab_notebook_contract.py` | 3 | constraints | `constraint`, `unit` | 3 | no | no | yes | 验证真实视频 workflow notebook 入口遵循受治理合同。 |
...
| `tests/functional/test_cross_event_vae_batching_config.py` | 4 | functional | `quick`, `unit` | 4 | no | no | yes | 验证 cross-event VAE batching 配置解析与批处理工具。 |
...
| `tests/functional/test_gpu_runtime_profiler.py` | 8 | functional | `quick` | 8 | yes | yes | yes | 验证 GPU runtime profiler 在无 GPU 与 stop-file 场景下的行为。 |
| `tests/functional/test_mechanism_audit.py` | 1 | functional | `quick` | 1 | yes | no | yes | 验证阶段 2 mechanism audit 能重建表格与决策文件。 |
| `tests/functional/test_mechanism_calibration_runner.py` | 1 | functional | `quick`, `unit` | 1 | yes | no | yes | 验证 real_video_vae_latent_probe mechanism calibration runner 的 orchestration 行为。 |
| `tests/functional/test_mechanism_candidate_selection.py` | 2 | functional | `quick` | 2 | yes | no | yes | 验证 real_video_vae_latent_probe mechanism calibration candidate selector 的 quick 行为。 |
...
| `tests/functional/test_real_video_probe_runner_profiles.py` | 16 | functional | `quick`, `unit` | 16 | yes | no | yes | 验证 real-video debug profile 的受治理配置解析。 |
| `tests/functional/test_real_video_probe_workflow.py` | 10 | functional | `quick`, `unit` | 10 | yes | yes | yes | 验证阶段 2 notebook workflow helper 的 manifest handoff 与 session model 语义。 |
...
| `tests/functional/test_sync_rescue_fusion.py` | 7 | functional | `quick`, `unit` | 7 | yes | no | no | Validate sync-rescue fusion semantics for the synthetic tubelet probe. |
