...
├── configs/
│   ├── method/
│   │   ├── tubelet_sync_v1.yaml
│   │   ├── trajectory_probe_v1.yaml
│   │   └── full_method_v1.yaml
│   ├── protocol/
│   │   ├── split_dev_calib_test.yaml
│   │   ├── fixed_low_fpr.yaml
│   │   └── attack_matrix_v1.yaml
│   ├── ablation/
│   │   ├── frame_prc.yaml
│   │   ├── tubelet_only.yaml
│   │   ├── tubelet_sync.yaml
...
method_validation_report_v1.md
```
...
8. `delta_traj = TPR(full) - TPR(tubelet_sync)`。
...
2. `tubelet_traj` 或 `full` 相比无 trajectory variant 存在边际增益。
...
2. full method 在至少部分攻击下优于不使用 sampling constraint 的版本。
...
delta_traj = TPR(full) - TPR(tubelet_sync)
```
...
```json
{
  "run_id": "string",
  "created_at": "string",
  "project_stage": "stage_0|stage_1|stage_2|stage_3|stage_4|stage_5|stage_6",
...
4. `method_validation_report_v1.md` 已生成。
...
4. Full Paper Protocol 所需方法变体和指标已齐备。
...
```text
构建 stage_0_protocol_skeleton_v1，并保证 video_tubelet_sync_probe_v1 能够作为阶段 1 的直接入口。
```
