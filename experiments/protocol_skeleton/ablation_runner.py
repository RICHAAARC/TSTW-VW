"""
文件用途：提供阶段 0 protocol_skeleton 的 experiment-scoped ablation runner。
File purpose: Provide the experiment-scoped ablation runner for the protocol_skeleton stage.
...
class AblationRunResult:
    """功能：定义 protocol_skeleton 运行结果载体。

    Protocol-skeleton run result payload.
...
class AblationRunner:
    """功能：执行阶段 0 protocol_skeleton 的共享 protocol。

    Protocol-skeleton ablation runner that executes all governed stage-zero method variants.
    ...
    def run(
        ...
        """功能：运行完整的 stage-zero 共享协议。

        Run the complete shared protocol for the protocol_skeleton stage.

        Args:
            output_root: Run root path.
            samples_per_role: Sample count per split-role pair.
            runtime_profile_override: Optional runtime profile override used only for
                governed logging and metadata continuity.
        ...
        Returns:
            A protocol-skeleton run result.
        """
        ...
        protocol_runner = ProtocolRunner(latent_backend=SyntheticLatentBackendRandom())
    ...
    def _resolve_protocol_config(
        ...
        resolved_protocol_config = copy.deepcopy(protocol_config)
        if runtime_profile_override is not None:
            ...
            resolved_protocol_config["runtime_profile"] = runtime_profile_override
        else:
            resolved_protocol_config.setdefault("runtime_profile", "smoke")
        return resolved_protocol_config
    ...
    def _resolve_runtime_method_config_paths(
        ...
        for method_config in runtime_method_configs:
            ...
            if method_variant in seen_variants:
                continue
            if method_variant not in method_config_paths:
                raise ValueError(
                    f"missing method config path for method_variant: {method_variant}"
                )
            seen_variants.add(method_variant)
            resolved_paths.append(method_config_paths[method_variant])
    ...
    def _emit_variant_complete(
        ...
        print(
            (
                ...
                f"{method_config['method_variant']} "
                f"events={variant_runtime_profile.event_count} "
